...
MAC_ROOT = os.environ.get("MAC_ROOT", "Desktop/ArionAgentProd")
...
if kill -0 "$APID" 2>/dev/null; then
  echo "Agent runner pid=$APID (mode=$DEPLOY_MODE)"
else
...
if kill -0 "$BPID" 2>/dev/null; then
  echo "Backend pid=$BPID port=$BACKEND_PORT"
else
  echo "Backend FAILED"
...
if kill -0 "$FPID" 2>/dev/null; then
  echo "Frontend pid=$FPID port=$FRONTEND_PORT"
else
...
echo "RESTART DONE mode=$DEPLOY_MODE ports=$BACKEND_PORT/$FRONTEND_PORT"
"""
